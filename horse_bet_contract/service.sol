...
contract Service {
    ...
    /// @notice utility function that generates random number between 0 and HORSES-1(both inclusive)
    function random(uint HORSES) private returns (uint) {
        randNonce++;
        return uint(keccak256(abi.encodePacked(block.timestamp,msg.sender,randNonce))) % HORSES;
    }
